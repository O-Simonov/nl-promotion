...
**Получить API-ключ — ⚠️ заморочка с картинками:**

Ключ СООБЩЕСТВА («Управление → Работа с API») умеет постить **только текст**.
На загрузке фото он падает: `error 27 — method is unavailable with group auth`.
Для постов **С КАРТИНКАМИ** нужен **ПОЛЬЗОВАТЕЛЬСКИЙ** токен аккаунта,
который является **админом** сообщества (бот всё равно постит от имени сообщества).

Создать своё приложение на dev.vk.com не выйдет (VK гонит в Бизнес ID с требованием ИНН),
готовый «VK Admin» (`client_id=6121396`) заблокирован. Рабочий способ (проверено 2026-06) — **Kate Mobile**:

1. Открыть ссылку в браузере под аккаунтом-**админом** сообщества и нажать «Разрешить»:
   ```
   ...
   ```
2. В адресной строке появится `...#access_token=vk1.a.XXXX&expires_in=0&user_id=...`
   (`expires_in=0` = токен бессрочный, это даёт scope `offline`).
3. Скопировать кусок между `access_token=` и `&expires_in` — это значение для `botToken`.

Подробности, проверка и запасной вариант (текст без фото) — в `vk-bot/README.md`.
...
## Персонализация — что заменить под себя (важно!)

Проект настроен под конкретного дистрибьютора. Коллеге нужно заменить личные данные на свои:

| Что заменить | Где |
|--------------|-----|
| Реф-ссылки NL (`nlstar.com/ref/...`, `nlstore.com/ref/...`) | `index.html`, `biznes.html`, `kak-zakazat.html`, `prezentaciya.html`, посты в `tg-bot/queue` и `tg-bot/backlog` |
| Контакты и соцсети (Telegram, VK, Instagram, OK, Facebook, MAX) | `index.html` |
| Имя и подпись (напр. «Олег Симонов», `@Simka1969_nl`) | `index.html`, презентации, `tg-bot/Make-PostImage.ps1` (подпись на картинках), посты |
| Фото `foto.jpg` | корень проекта |
| Адрес сайта (`oleg-nl.netlify.app`) | посты и презентации — после деплоя на свой Netlify |
| OG-обложки `og-*.png` | при желании перегенерировать под своё имя |

Быстро найти все вхождения чужих данных, чтобы ничего не пропустить:
```powershell
Select-String -Path *.html, tg-bot\queue\*.txt, tg-bot\backlog\*.txt `
  -Pattern "nlstar.com/ref|nlstore.com/ref|Simka1969|oleg-nl|Олег Симонов"
```

---

## Как работает очередь (важно понимать)

`tg-bot/queue/` — **одна общая очередь для всех 4 ботов**. Каждый бот по расписанию берёт **следующий** пост из очереди, публикует его в свою сеть и перекладывает в `<бот>/sent/`.

➡️ Это значит: в один день в разные сети уходят **разные** посты (Telegram — один, Facebook — следующий, и т.д.), а **не один и тот же во все сразу**. Так контент не дублируется и выглядит разнообразно.

Если нужно публиковать **один и тот же** пост во все 4 сети одновременно — это другая логика, её нужно дорабатывать отдельно.
